...
namespace Photon.Pun.Demo.PunBasics
{
    ...
    public class PlayerManager : MonoBehaviourPunCallbacks, IPunObservable
    {
        ...
        public void Start()
        {
            var camFollow = gameObject.GetComponent<ThirdPersonFollowCamera>();
            if (camFollow != null && photonView.IsMine)
            {
                // 별도 초기화 없이 Start/LateUpdate에서 자동 동작
                // (Camera.main을 자동 획득, 타깃=this.transform)
            }
            else if (camFollow == null)
            {
                Debug.LogWarning("ThirdPersonFollowCamera missing on Player prefab.");
            }
